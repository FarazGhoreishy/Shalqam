...
class Ui_Dialog(object):
    def setupUi(self, Dialog, search_entry, items):
        ...
        # Creating entry label at the top of the page
        self.search_entry_label = QtWidgets.QLabel(Dialog)
        font = QtGui.QFont()
        font.setFamily("Josefin Slab")
        font.setPointSize(28)
        font.setBold(True)
        font.setWeight(75)
        self.search_entry_label.setFont(font)
        self.search_entry_label.setStyleSheet("QLabel{ width: 100%; height: 55px; background-color: #CA80DC; border-radius: 15px; padding: 0 35px 0 35px; color: #fff; line-height: 1.2;}")
        self.search_entry_label.setAlignment(QtCore.Qt.AlignCenter)
        self.search_entry_label.setObjectName("search_entry_label")
        self.search_entry_label.setText(_translate("Dialog", search_entry))

        self.verticalLayout.addWidget(self.search_entry_label)
        ...
        rows = len(items)
        positions = [(i, 0) for i in range(rows)]
